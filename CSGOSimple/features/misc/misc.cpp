...
void c_misc::create_move(CUserCmd* cmd, bool* bSendPacket) {
	...
	if (options.grenade_prediction) GrenadePrediction::trace(cmd);
...
void c_misc::paint() {
	...
	if (options.grenade_prediction) GrenadePrediction::draw();
...
void c_misc::slow_walk(CUserCmd* pCmd) {
	if (!options.slow_walk && !CheatGVars::SlowWalking) {
		return;
	}
...
void c_misc::Menu() {
	...
	ImGui::BeginChild("##misc", ImVec2(-1, -1), true); {
		...
		ImGui::Checkbox("EdgeJump", &options.edge_jump);
		...
		ImGui::Checkbox("SlowWalk", &options.slow_walk);
		...
		ImGui::Checkbox("3rd Person", &options.third_person);
		...
		if (ImGui::Button("Reset Size", ImVec2(-1, 19))) {
			g_ChangeMenuSizeValue = ImVec2(976, 450);
	...
	ImGui::BeginChild("##miscoptions", ImVec2(-1, -1), true); {
		...
		if (options.bhop && ImGui::SliderInt("BHop Chance", &options.bhop_chance, 1, 100))
		...
		if (options.knife_bot) {
			if (ImGui::SliderInt("KB Distance", &options.knife_bot_dist, 1, 100))
				options.knife_bot_dist = std::clamp(options.knife_bot_dist, 1, 100);
			ImGui::Combo("KB Filter", &options.knife_bot_filter, values::knifebot_filters, 3);
			ImGui::Combo("KB Mode", &options.knife_bot_mode, values::knifebot_modes, 4);
		}
		...
		if (options.slow_walk && ImGui::SliderFloat("SW Speed", &options.slow_walk_speed, 26.f, 100.f))
		...
		if (options.legit_aa) {
			ImGui::Combo("AA Mode", &options.legit_aa_mode, values::legitaa_modes, 4);
		...
		if (options.resolver_enabled) 
			ImGui::Combo("Resolver Mode", &options.resolver_mode, values::resolver_modes, 1);
	};
	ImGui::EndChild();
	ImGui::NextColumn();
	...
	ImGui::BeginChild("##colorsmisc", ImVec2(-1, -1), true); {
		ImGui::ColorEdit("Menu accent", colors.accent_color);
		if (options.show_fov) {
