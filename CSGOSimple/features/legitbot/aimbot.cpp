...
void Aimbot::TriggerBot(CUserCmd* pCmd) {
 	if (!settings.trigger_enable) return;

	if (trigger_onkey && !trigger_keybind_pressed) {
		is_delayed = false;
		shot_delay = false;
		return;
	}
	...
	if (g_LocalPlayer->m_hActiveWeapon()->m_Item().m_iItemDefinitionIndex() != WEAPON_TASER) {
		if (settings.trigger_hitgroup_head)
			hitgroups.push_back(HITGROUP_HEAD);
		if (settings.trigger_hitgroup_chest)
			hitgroups.push_back(HITGROUP_CHEST);
		if (settings.trigger_hitgroup_stomach)
			hitgroups.push_back(HITGROUP_STOMACH);
		if (settings.trigger_hitgroup_left_arm)
			hitgroups.push_back(HITGROUP_LEFTARM);
		if (settings.trigger_hitgroup_right_arm)
			hitgroups.push_back(HITGROUP_RIGHTARM);
		if (settings.trigger_hitgroup_left_leg)
			hitgroups.push_back(HITGROUP_LEFTLEG);
		if (settings.trigger_hitgroup_right_leg)
	...
	if (!player || player->IsDormant() || !player->IsAlive() || !player->IsPlayer() || player->m_bGunGameImmunity())
		return;

	if (player->m_iTeamNum() == g_LocalPlayer->m_iTeamNum() && !trigger_deathmatch)
		return;

	if (IsLineGoesThroughSmoke(g_LocalPlayer->GetEyePos(), tr.endpos) && settings.trigger_check_smoke)
		return;

	if (g_LocalPlayer->IsFlashed() && settings.trigger_check_flash)
		return;

	if (dh && HitChance(viewangles, player, settings.trigger_hit_chance))
	{
		if (!is_delayed_trigger && !shot_delay_trigger && settings.trigger_delay > 0) {
			is_delayed_trigger = true;
			shot_delay_trigger = true;
			shot_delay_time_trigger = g_GlobalVars->curtime + settings.trigger_delay;
		}

		if (shot_delay_trigger && shot_delay_time_trigger <= g_GlobalVars->curtime) {
			shot_delay_trigger = false;
		}

		if (!shot_delay_trigger)
		{
			is_delayed_trigger = false;
			shot_delay_trigger = false;
			pCmd->buttons |= IN_ATTACK;
		}
	}
...
void Aimbot::MenuAimbot() {
	...
	ImGui::BeginChild("##2", ImVec2(-1, -1), true); {
		...
		if (ImGui::Checkbox("PSilent", &settings->psilent) && settings->psilent)
	...
	ImGui::BeginChild("##4", ImVec2(-1, -1), true); {
		...
		if (!settings->silent)
			if (ImGui::SliderInt("Shot Delay", &settings->shot_delay, 0, 100))
				settings->shot_delay = std::clamp(settings->shot_delay, 0, 100);
...
void Aimbot::MenuTrigger() {
	static bool editor_mode{ false };
	static std::string current_group{ "" };

	if (Menu::Get().TriggerFirstTimeRender) {
		if (g_LocalPlayer && g_EngineClient && g_EngineClient->IsInGame() && g_EngineClient->IsConnected()) {
			auto weapon = g_LocalPlayer->m_hActiveWeapon();
			if (weapon) {
				current_group = g_CustomWeaponGroups->GetGroupName(weapon->m_Item().m_iItemDefinitionIndex());
	...
	ImGui::BeginChild("##2", ImVec2(-1, -1), true); {
		...
		ImGui::Checkbox("Friendly Fire##trigger", &trigger_deathmatch);
		ImGui::Checkbox("On key##trigger", &trigger_onkey);
		if (trigger_onkey)
			ImGui::Keybind("##trigger", &trigger_keybind);
	...
	ImGui::ListBoxHeader("##trigger_hitgroups", ImVec2(-1, -1)); {
		ImGui::Selectable("Head", &settings->trigger_hitgroup_head, NULL, ImVec2(0, 0));
		ImGui::Selectable("Chest", &settings->trigger_hitgroup_chest, NULL, ImVec2(0, 0));
		ImGui::Selectable("Stomach", &settings->trigger_hitgroup_stomach, NULL, ImVec2(0, 0));
		ImGui::Selectable("Left Arm", &settings->trigger_hitgroup_left_arm, NULL, ImVec2(0, 0));
		ImGui::Selectable("Right Arm", &settings->trigger_hitgroup_right_arm, NULL, ImVec2(0, 0));
		ImGui::Selectable("Left Leg", &settings->trigger_hitgroup_left_leg, NULL, ImVec2(0, 0));
		ImGui::Selectable("Right Leg", &settings->trigger_hitgroup_right_leg, NULL, ImVec2(0, 0));
	};
	ImGui::ListBoxFooter();

	Menu::Get().TriggerFirstTimeRender = false;
}
...
void Aimbot::SetupValues() {
	...
	g_Config->PushItem(&trigger_deathmatch, "trigger", "deathmatch", trigger_deathmatch);
	g_Config->PushItem(&aimbot_on_key, "aim", "on_key", aimbot_on_key);
	g_Config->PushItem(&trigger_onkey, "trigger", "on_key", trigger_onkey);
	g_Config->PushItem(&aimbot_key, "aim", "key", aimbot_key);
	g_Config->PushItem(&trigger_keybind, "trigger", "key", trigger_keybind);
}
